...
def test_precise_delete():
    """Test the improved delete logic to see which objects are actually affected"""
    
    # Test deleting a sector that should only affect a few objects
    sector_to_delete = "Energy & Utilities"
    
    print(f"🧪 Testing delete logic for sector: {sector_to_delete}")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print(f"🗑️  Testing delete of sector: {sector_to_delete}")
    
    # Test the delete endpoint
    try:
        response = requests.delete(f"http://localhost:8000/api/v1/drivers/sectors/{sector_to_delete}")
