    ...
    try:
        with driver.session() as session:
            ...
            discrete_id_result = session.run("""
                ...
                RETURN v.id as variableId, v.name as variableName, p.name as part, s.name as section, coalesce(v.group, '') as group
            """, object_id=object_id)
        ...
        with driver.session(default_access_mode=WRITE_ACCESS) as session:
            ...
            if existing_avatar:
                ...
                if different_being_avatar:
                    ...
                    avatar_id = str(uuid.uuid4())
                    print(f"Note: Avatar '{object_data.avatar}' exists for different Being, creating NEW avatar node with ID '{avatar_id}' for Being '{object_data.being}'")
                    session.run("""
                        CREATE (a:Avatar {
                            id: $avatar_id,
                            name: $avatar,
                    ...
                    """, avatar_id=avatar_id, avatar=object_data.avatar, being=object_data.being)
                    ...
                    """, being=object_data.being, avatar_id=avatar_id)
                else:
            ...
            # Create Object and link to Avatar using avatar ID
            session.run("""
                MATCH (a:Avatar {id: $avatar_id})
                MERGE (o:Object {id: $object_id, name: $object_name})
                MERGE (a)-[:HAS_OBJECT]->(o)
            """, avatar_id=avatar_id, object_id=new_id, object_name=object_data.object)
                        ...
                        if variable_ids:
                            ...
                            if 'ALL' in variable_ids:
                                all_vars_result = session.run("""
                                    MATCH (p:Part {name: 'Identifier'})-[:HAS_SECTION]->(:Section)-[:HAS_VARIABLE]->(v:Variable) WHERE coalesce(v.group, '') = 'Public ID'
                                    RETURN v.id as variableId
                                """, object_id=new_id)
                            ...
                            if isinstance(entry, dict):
                                ...
                                if part and group and variable_id:
                                    if section == 'ANY':
                                        ...
                                        all_vars_result = session.run("""
                                            MATCH (p:Part {name: $part})-[:HAS_SECTION]->(s:Section)-[:HAS_VARIABLE]->(v:Variable)
                                        ...
                                        if section:
                                            ...
                                            all_vars_result = session.run("""
                                                MATCH (p:Part {name: $part})-[:HAS_SECTION]->(s:Section {part_name: $part, name: $section})-[:HAS_VARIABLE]->(v:Variable)
                                                RETURN DISTINCT v.id as variableId
                                        ...
                                        if section:
                                            all_vars_result = session.run("""
                                                MATCH (p:Part {name: $part})-[:HAS_SECTION]->(s:Section {part_name: $part, name: $section})-[:HAS_VARIABLE]->(v:Variable) WHERE coalesce(v.group, '') = $group
                                                RETURN v.id as variableId
                                            """, part=part, group=group, section=section)
                                        else:
                                            all_vars_result = session.run("""
                                                MATCH (p:Part {name: $part})-[:HAS_SECTION]->(s:Section)-[:HAS_VARIABLE]->(v:Variable)
                                                WHERE coalesce(v.group, '') = $group
                                                RETURN v.id as variableId
                    ...
                    if being_changed or avatar_changed or object_changed:
                        ...
                        if final_result:
                            ...
                            if final_being and final_avatar:
                                ...
                                # Capture the old avatar node(s) for this object (any avatar other than
                                # the new target) BEFORE unlinking, so we can clean up empties afterwards.
                                old_avatar_eids = session.run("""
                                    MATCH (old_a:Avatar)-[:HAS_OBJECT]->(o:Object {id: $object_id})
                                    WHERE (old_a.id IS NULL OR old_a.id <> $avatar_id)
                                    RETURN collect(DISTINCT elementId(old_a)) AS eids
                                """, object_id=object_id, avatar_id=avatar_id).single()["eids"]
                                session.run("""
                                    MATCH (old_a:Avatar)-[r:HAS_OBJECT]->(o:Object {id: $object_id})
                                    WHERE (old_a.id IS NULL OR old_a.id <> $avatar_id)
                                    DELETE r
                                """, object_id=object_id, avatar_id=avatar_id)
                                # If a previously-linked avatar now has no objects at all, remove it
                                # (DETACH also drops its Being-[:HAS_AVATAR]->() link) so the old Being
                                # isn't left with empty, dangling avatars after a bulk Being move.
                                if old_avatar_eids:
                                    session.run("""
                                        MATCH (old_a:Avatar)
                                        WHERE elementId(old_a) IN $eids
                                          AND NOT (old_a)-[:HAS_OBJECT]->(:Object)
                                        DETACH DELETE old_a
                                    """, eids=old_avatar_eids)
            ...
            for row_num, row in enumerate(csv_reader, start=2):  # Start at 2 because of header
                try:
                    ...
                    session.run("""
                        ...
                        MATCH (o:Object {id: $object_id})
                        MERGE (a)-[:HAS_OBJECT]->(o)
                    """, avatar_id=avatar_id, object_id=new_id)
...
async def create_avatar(being: str = Body(...), avatar: str = Body(...)):
    ...
    try:
        with driver.session() as session:
            ...
            # Generate unique ID for the new avatar node
            avatar_id = str(uuid.uuid4())
            ...
            different_being_check = session.run("""
                MATCH (b:Being)-[:HAS_AVATAR]->(a:Avatar {name: $avatar})
                WHERE b.name <> $being
                RETURN b.name as being_name, a.id as avatar_id
                LIMIT 1
            """, being=being, avatar=avatar).single()
            
            if different_being_check:
                # Avatar with same name exists for different being - create a NEW avatar node with unique ID
                print(f"Note: Avatar '{avatar}' exists for Being '{different_being_check['being_name']}', creating NEW avatar node with ID '{avatar_id}' for Being '{being}'")
                
                # Create a NEW Avatar node with unique ID and being property
                session.run("""
                    CREATE (a:Avatar {
                        id: $avatar_id,
                        name: $avatar,
                ...
                """, avatar_id=avatar_id, avatar=avatar, being=being)
            else:
                # No existing avatar with this name for a different being
                # Check if one exists for the same being (should have been caught above, but double-check)
                same_being_check = session.run("""
                    MATCH (b:Being {name: $being})-[:HAS_AVATAR]->(a:Avatar {name: $avatar})
                    RETURN a.id as avatar_id
                    LIMIT 1
                """, being=being, avatar=avatar).single()
                ...
                if existing_avatar_no_id:
                    ...
                    # No existing avatar at all - create new one
                    session.run("""
                        CREATE (a:Avatar {
                            id: $avatar_id,
                            name: $avatar,
            ...
            # Create the relationship from Being to Avatar
            session.run("""
                MATCH (b:Being {name: $being})
                MATCH (a:Avatar {id: $avatar_id})
                MERGE (b)-[:HAS_AVATAR]->(a)
            """, being=being, avatar_id=avatar_id)
        ...
        with driver.session() as session:
            ...
            for i in range(1, 6):
                composite_result = session.run(f"""
                    ...
                    MATCH (p:Part)-[:HAS_SECTION]->(:Section)-[:HAS_VARIABLE]->(v)
                    RETURN v.id as variableId, p.name as part, coalesce(v.group, '') as group
                """, source_id=source_object_id).data()
