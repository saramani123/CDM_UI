...
export const TieredListValuesModal: React.FC<TieredListValuesModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && selectedList && columnHeaders.length >= 1) {
      ...
      // Reset when modal closes - but don't clear rows immediately
      // Wait a bit to allow the parent to save state before clearing
      console.log('🔄 Modal closed, resetting flag');
      hasLoadedInitialData.current = false; // Reset flag so we load again when reopening
      setEditingCell(null);
      setEditValue('');
      // Don't clear rows immediately - let the parent component handle state
      // The rows will be reloaded when the modal reopens
    }
                        ...
                        <div className="flex items-center">
                          {editingCell?.rowId === row.id && editingCell?.colIndex === colIndex && !editingCell?.isVariation ? (
                            <input
                              ...
                              onKeyDown={(e) => handleCellKeyDown(e, row.id, colIndex, false, false)}
                              onPaste={(e) => handlePaste(e, row.id, colIndex, false, false)}
                              autoFocus
