...
export const AddVariablePanel: React.FC<AddVariablePanelProps> = ({
  ...
  // Modal state for add group values (Part/Section creation is centralized in the Metadata tab)
              ...
              {validationComponents.valType === 'Range' && (
                <>
                  ...
                  <div>
                    ...
                    <input type="text" value={validationComponents.lessThanValue ?? ''} onInput={(e) => { const v = (e.target as HTMLInputElement).value; const res = validateValidationInput('Range', v, formData.formatI, formData.formatII); setValidationError(res.isValid ? '' : (res.error || '')); setValidationComponentsList(prev => prev.map((comp, i) => i === index ? { ...comp, lessThanValue: v } : comp)); }} placeholder={formData.formatI === 'Date' || formData.formatI === 'Time' ? 'e.g. 3/8/2026' : 'Enter value'} className={`w-full px-3 py-2 bg-ag-dark-bg border border-ag-dark-border rounded text-ag-dark-text focus:ring-2 focus:ring-ag-dark-accent ${validationError && validationError.includes('Value') ? 'border-red-500' : ''}`} />
                    {validationError && validationError.includes('Value') && <p className="mt-1 text-sm text-red-500">{validationError}</p>}
                  </div>
                </>
              )}
