...
interface CloneRelationshipsModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const CloneRelationshipsModal: React.FC<CloneRelationshipsModalProps> = ({
  isOpen,
  onClose,
  targetObject,
  targetObjects = [],
  ...
  const handleClone = async () => {
    if (!selectedSourceObject) {
      setError('Please select an object to clone relationships from');
      return;
    }
    ...
    try {
      if (isBulkMode) {
        ...
        await apiService.bulkCloneRelationships(selectedSourceObject.id, targetIds);
      } else {
        // Single clone: clone to one target object
        await apiService.cloneRelationships(effectiveTargetObjects[0].id, selectedSourceObject.id);
      }
      ...
      console.error('Error cloning relationships:', err);
    } finally {
      setCloning(false);
    }
  };
      ...
      <div className="bg-ag-dark-surface border border-ag-dark-border rounded-lg shadow-xl w-full max-w-6xl h-[80vh] flex flex-col">
        ...
        <div className="p-4 bg-ag-dark-bg border-b border-ag-dark-border">
          ...
          <div className="text-sm text-ag-dark-text-secondary mt-1">
            Select an object below to clone its relationships to the {isBulkMode ? 'selected objects' : 'target object'}.
          </div>
        </div>
