...
export const ListsOntologyModal: React.FC<ListsOntologyModalProps> = ({
  ...
  const loadGraph = async () => {
    ...
    try {
      ...
      // For metadata view, auto-select the first list node and expand panel
      if (viewType === 'metadata' && graphData.nodes.length > 0) {
      ...
      const nodes: Node[] = graphData.nodes.map((node: any) => {
        ...
        const nodeColor = isSelected && (viewType === 'drivers' || viewType === 'metadata') 
          ? (viewType === 'metadata' 
  ...
  const getCypherQuery = () => {
    // This would be generated by the backend, but for now return a placeholder
    // The actual query will be returned from the API response
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-ag-dark-surface rounded-lg border border-ag-dark-border w-[90vw] h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-ag-dark-border">
          <div>
            <h2 className="text-xl font-semibold text-ag-dark-text">Ontology View - {sectionName}</h2>
